...
- **Lint-before-done**: `mr l` must pass before reporting a task complete.
...
- `.chezmoiignore` keeps repo-only files (the `/dot` Go CLI, `/skills`, `/AGENTS.md`, `README.md`, `LICENSE`, `install.sh`, `mise.toml`, `lefthook.yml`, `dprint.json`, `go.work`/`go.work.sum`, `skill-lock.json`) out of `apply`, plus the Ghostty `.desktop` file on non-Linux hosts and `secrets.fish` when the age key is absent.
...
- **Skills** — `skills/` (at the root of this repository) is the canonical home and is symlinked to `~/.agents/skills/` via chezmoi symlink templates.
  - GitHub Copilot CLI automatically discovers skills from `~/.agents/skills/` natively.
  - Antigravity CLI and Claude consume them via symlink templates (`dot_gemini/antigravity-cli/symlink_skills.tmpl` and `dot_claude/symlink_skills.tmpl`) pointing to `~/.agents/skills/`.
  - Claude also consumes project-scoped skills in this repository via the `.claude/skills` symlink pointing to `../skills` (so running Claude Code in the repository loads the project's skills).
  - OpenCode auto-loads them natively from `~/.agents/skills/<name>/SKILL.md` (its skill loader scans `~/.agents/skills` and `~/.claude/skills`), so it needs no skills symlink of its own.
...
- `.claude/` — Workspace-scoped session records and state for the Claude Code CLI (holds the project skills symlink at `.claude/skills` → `../skills`).
...
- `skill-lock.json` — Version tracking lock file for external Agent Skills.
