...
- **Precedence**: Project-level instructions override this file; on conflict, follow the project and mention the deviation.
...
- **Signal Over Noise**: Cut filler, restatement of the request, and narration of the steps you took. Prefer short headings, tight lists, and bold labels over prose; prefer a table when comparing three or more items.
...
- **Infrastructure**: Use the [terraform-stack](~/.agents/skills/terraform-stack/SKILL.md) skill for all infrastructure-as-code work — OpenTofu (`tofu`) is the default engine.
...
- **Documents**: Use the [typst](~/.agents/skills/typst/SKILL.md) skill for standalone documents (papers, reports, CVs) — Typst replaces LaTeX and Word.
- **Sites & Docs**: Use the [hugo](~/.agents/skills/hugo/SKILL.md) skill for documentation sites, project docs, and static websites (Hugo extended + Hextra) — Go web _applications_ stay on the go-stack GOTH setup.
...
- **Cloud Deployment**: Use the [cloud-run](~/.agents/skills/cloud-run/SKILL.md) skill to ship services and agents to GCP — keyless CI deploys via Workload Identity Federation.
...
- **Secrets Management**: Use the [sops-secrets](~/.agents/skills/sops-secrets/SKILL.md) skill (sops + age) for secrets in git and at runtime — encrypted `*.enc.*` files, memory-only decryption.
