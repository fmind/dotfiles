...
- **Fix Root Causes, Report Honestly**: Fix the underlying cause — never ship a quick or dirty fix that masks a symptom to force a green result (weaken assertions, add skips/`xfail`, loosen a type, suppress a lint error, blanket-exclude a path); if only a shortcut fits the moment, say so and propose the real fix instead of shipping it. Surface failing tests, broken builds, and dead ends plainly.
