...
1. **Validate**: `git init --initial-branch=main`, then `mise run install`, `mise run format`, `mise run check`, `mise run test`, `mise run build`; `check:leaks` reports zero commits until the first commit.
