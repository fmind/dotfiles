...
1. Run the cross-cutting repository review across the requested source, tests, tooling, CI, security, documentation, generated state, release, and authorized runtime dimensions. Retain partial scans and unavailable services as evidence gaps.
1. Fetch every open and closed issue needed for duplicate analysis, including complete bodies, comments, labels, state, and native `blockedBy`/`blocking` relationships. Search by the underlying problem and evidence, not title similarity alone.
1. Research current primary documentation or upstream source only where it can materially confirm a retained finding or implementation boundary. Record the publication/version context and distinguish sourced fact from inference.
...
1. Draft every retained item with the complete contract in [draft-contract.md](references/draft-contract.md). Explain why it is distinct from every close match and model dependencies as draft-to-draft or draft-to-existing-issue edges.
...
1. Require an explicit instruction to create the reviewed draft set and reconfirm the exact `owner/repo`. Authorization to review, draft, implement code, or create a PR is not authorization to create backlog issues.
1. Refresh repository visibility, labels, matching open/closed issues, and native dependencies immediately before mutation. Stop if the drafts became stale or the required `area/*`, single `priority/p*`, and single `effort/*` labels do not exist.
...
1. Create no dependency edges until every issue node exists. If issue creation is partial, stop without deleting successful issues, report created/failed/unattempted drafts, and leave the graph unapplied.
...
1. Verify every created issue body, label set, and `blockedBy`/`blocking` relationship from GitHub. Return a mutation receipt; do not claim completion from successful issue creation alone.
