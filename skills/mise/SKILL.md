...
- **Fast feedback is not completion proof**: A project may expose `mise run fast` for deterministic changed-file checks, focused tests, and compilation. Print the selected checks, fall back to the broader static gate on uncertainty, and keep `mise run all` plus exact-head CI as the completion boundary.
